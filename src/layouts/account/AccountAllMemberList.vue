              ...
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td>
                  ...
                  {{ tr.first_name +' '+ tr.last_name }}<br>
                    ...
                    <vs-checkbox v-model="tr.marketing" @click="changeAccType(tr.id, tr.marketing)">Marketing</vs-checkbox>
                  </p>
                  <p>
                ...
                <vs-td>
                  ...
                  <vs-chip :color="getOrderStatusColor(tr.active_2fa)" class="user-order-status">{{ getOrderStatusColorText(tr.active_2fa) | title }}</vs-chip>
                </vs-td>
          ...
          if(resp.data.success){
              ...
              for(let obj in data) {
                ...
                for(let i = 0; i < data[obj].length; i++){
                  this.treeData[0].children.push({title: `<span style='color: white'>Cấp ${c} - ${data[obj][i].nick_name} - Khối lượng Giao Dịch: ${ this.formatPrice(data[obj][i].tklgd, 2)}</span>`})
                }
              }   
          }
        })
